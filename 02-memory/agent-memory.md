...
上一篇我们搭出了 Agent 的最小闭环：**LLM + 工具 + 循环**。但那个版本还有两个致命缺陷：没有记忆，也不会规划。

真正一上手就会发现，很多任务不是“工具会不会调”的问题，而是“上一次做了什么还记不记得”“当前任务要不要先拆开”。如果这两件事处理不好，Agent 就只能停留在单次响应器阶段。

今天我们看 nanoagent 的第二个版本 [agent-memory.py](./agent-memory.py)（当前仓库中 `206` 行），它主要解决两个根本问题：**让 Agent 记住过去**，**让 Agent 规划未来**。

## 本文聚焦

- 对应脚本：[agent-memory.py](./agent-memory.py)
- 当前仓库中的脚本行数：`206`
- 这篇会回答三件事：
- Agent 的“记忆”为什么本质上仍然是 prompt 注入
- 规划为什么不是魔法，而是一次额外的模型调用
- 多步任务执行时，步骤之间如何共享上下文
...
| 能力 | agent-essence.py (103 行) | agent-memory.py (206 行) |
|------|----------|---------------|
| 工具调用 | ✅ 3 个工具 | ✅ 3 个工具（不变） |
| 单任务执行 | ✅ | ✅ |
...
工具层几乎没变，新增代码主要都用来构建更高层的能力。这恰好印证了第一篇的结论：工具 + 循环只是地基，记忆和规划才是让 Agent 真正可用的关键。
...
├────────────────────────────────────────────────────────────┤
│ 第 1 次运行：创建 hello.py                                 │
│   ↓ save_memory() -> 写入 agent_memory.md                  │
│ 第 2 次运行：读取 hello.py 并加注释                        │
│   ↓ load_memory() -> 注入 Previous context                 │
│ Agent 因此知道 hello.py 已经存在                           │
└────────────────────────────────────────────────────────────┘
```
...
│ ReAct：思考 -> 行动 -> 观察                                  │
│   └─ 每一步都边做边想，灵活但容易迷失                        │
│ Plan-then-Execute：先规划 -> 步骤 1 -> 步骤 2 -> 步骤 3      │
│   └─ 每步内部仍是 ReAct，但多了全局视角                      │
└──────────────────────────────────────────────────────────────┘
```
...
这三个问题，引出了 Agent 架构里下一层最关键的三个概念：**MCP（工具协议）**、**Rules（行为规则）**、**Skills（技能注册）**。在 [第三篇：Rules、Skills 与 MCP](../03-skills-mcp/agent-skills-mcp.md) 中，我们会看到 `agent-skills-mcp.py` 如何把一个“能跑”的 Agent，推进为一个“能配置、能扩展、能被项目约束”的 Agent harness。

---

*本文基于本仓库中的 [agent-memory.py](./agent-memory.py) 分析。如果你还没有读过系列第一篇，建议先从 [最小闭环与工具调用](../01-essence/agent-essence.md) 开始。*
