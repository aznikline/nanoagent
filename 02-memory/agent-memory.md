...
上一篇我们搭出了 Agent 的最小闭环：**LLM + 工具 + 循环**。但那个版本还有两个硬缺口：不会记住过去，也不会提前拆任务。

真正一上手就会发现，很多任务卡住的不是“工具会不会调”，而是“上一次做了什么还记不记得”“这件事要不要先拆开再做”。这两件事处理不好，Agent 就只能停留在单次响应器阶段。

这一篇看 `nanoagent` 的第二个版本 [agent-memory.py](./agent-memory.py)（当前仓库中 `206` 行），重点补上两个能力：**让 Agent 记住过去**，**让 Agent 规划未来**。

## 本文聚焦

- 对应脚本：[agent-memory.py](./agent-memory.py)
- 当前仓库中的脚本行数：`206`
- 核心问题 1：Agent 的“记忆”为什么本质上仍然是 prompt 注入
- 核心问题 2：规划为什么不是魔法，而是一次额外的模型调用
- 核心问题 3：多步任务执行时，步骤之间如何共享上下文
...
这三个问题，引出了 Agent 架构里下一层最关键的三个概念：**MCP（工具协议）**、**Rules（行为规则）**、**Skills（技能注册）**。在 [第三篇：Rules、Skills、MCP 与可配置能力](../03-skills-mcp/agent-skills-mcp.md) 中，我们会看到 `agent-skills-mcp.py` 如何把一个“能跑”的 Agent，推进为一个“能配置、能扩展、能被项目约束”的 Agent harness。
