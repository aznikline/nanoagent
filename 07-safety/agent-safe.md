...
1. 防线 1，命令黑名单：像 `rm -rf /` 这样的命令直接拦截，不进入确认环节。
2. 防线 2，用户确认：像 `find . -name '*.py'` 这样的命令交给用户决定放行、跳过还是终止。
3. 防线 3，输出截断：像 10000 行日志这类结果会被裁剪，避免把上下文窗口撑爆。
4. 最终结果再返回给 LLM，用于下一轮决策。
