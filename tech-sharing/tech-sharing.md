...
├──────────────────────────────────────────────────────────────┤
│ send：alice -> bob                                           │
│   └─ 仅进入 bob 的 inbox，适合传递特定信息                   │
│ broadcast：alice -> [bob, carol, dave, ...]                  │
│   └─ 进入所有成员的 inbox，适合同步全局进展                  │
└──────────────────────────────────────────────────────────────┘
```
...
│ 生命周期时序图                                               │
├──────────────────────────────────────────────────────────────┤
│ alice：hire -> chat(写后端) -> broadcast -> chat(修 bug)     │
│ bob：hire -> inbox -> chat(写前端) -> broadcast              │
│ carol：hire -> inbox -> chat(审查) -> disband                │
│ 信息通过 inbox 流动，而每个 Agent 的记忆独立累积             │
└──────────────────────────────────────────────────────────────┘
```
...
├──────────────────────────────────────────────────────────────────────────┤
│ 1. 命令黑名单：rm -rf / -> 直接拦截                                      │
│ 2. 用户确认：find . -name "*.py" -> Y / N / Q                            │
│ 3. 输出截断：超长结果裁掉中间部分，避免撑爆 context                      │
│ 4. 三层都通过后，命令才会真正执行                                        │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│ Agent 完整架构                                                                 │
├────────────────────────────────────────────────────────────────────────────────┤
│ 第七篇：安全防线 -> 黑名单、确认、截断、Hook                                   │
│ 第六篇：上下文压缩 -> Compaction                                               │
│ 第五篇：Teams -> 持久身份、通信、生命周期                                      │
│ 第四篇：SubAgent -> 一次性委派、独立上下文                                     │
│ 第三篇：Rules / Skills / MCP -> 行为约束与工具扩展                             │
│ 第二篇：Memory / Planning -> 记忆与任务拆解                                    │
...
*「从零开始理解 Agent」系列 —— 7 篇文章，7 个代码文件，1 篇番外，从最小闭环一路拆到完整 Harness。*
