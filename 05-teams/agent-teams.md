...
> - **第五篇：持久智能体与团队协作**（本文）—— 270 行
> - [第六篇：上下文压缩与长任务续航](../06-compact/agent-compact.md) —— 169 行
> - [第七篇：执行边界、安全确认与 Hook 化](../07-safety/agent-safe.md) —— 219 行

上一篇我们实现了 SubAgent。主 Agent 可以临时派出一个“专家”去做局部任务，但 SubAgent 的本质也很明确：**一次性临时工，生成 → 干活 → 返回摘要 → 消亡，没有身份，也没有记忆。**

这在很多场景下够用，但现实里的软件团队并不是这么运转的：后端工程师写完 API 后，前端工程师要知道接口长什么样；测试发现 bug 后，要把问题重新交回开发；修完之后，测试还得继续接着验。**同一个人，会被多次找到，而且他还记得上次做了什么。**

SubAgent 做不到这些，因为每次调用都是一个全新的、失忆的临时工。

这一篇补上的，就是**从临时委派升级为持久协作**。

## 本文聚焦

- 对应脚本：[agent-teams.py](./agent-teams.py)
- 当前仓库中的脚本行数：`270`
- 核心问题 1：为什么 `messages` 的存放位置会直接决定 Agent 的生命周期
- 核心问题 2：团队协作最少需要补上哪三类能力
- 核心问题 3：SubAgent 和 Teams 分别适合什么任务形态
...
## 七、本章结论：为什么 Teams 不是更大的 SubAgent
