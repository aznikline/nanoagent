...
| 阶段 | `messages` 里保留什么 |
|------|----------------------|
| 压缩前 | `system`、最初任务、早期大量 `assistant/tool` 记录、最近几轮操作 |
| 压缩时 | 把中间那段旧历史交给 `compact_messages()` 做摘要 |
| 压缩后 | `system`、一对“摘要消息”、最近 6 条原始消息 |
...
压缩前：
- system：永远保留
- 早期历史：bash 输出、read_file 内容、grep 结果等长记录
- 最近 6 条：当前仍在推进的关键上下文

压缩后：
- system：继续保留
- 摘要消息：例如“之前找到了 42 个 Python 文件，统计了行数……”
- 最近 6 条：完整保留，不做压缩
```
...
*本文基于 [agent-compact.py](./agent-compact.py) 分析。完整系列：[第一篇](../01-essence/agent-essence.md) → [第二篇](../02-memory/agent-memory.md) → [第三篇](../03-skills-mcp/agent-skills-mcp.md) → [第四篇](../04-subagent/agent-subagent.md) → [第五篇](../05-teams/agent-teams.md) → 第六篇（本文） → [第七篇](../07-safety/agent-safe.md)*
