...
很多人用过 ChatGPT、Claude 这样的对话式 AI，也听说过 AI Agent 这个概念。但一旦问题变成“它为什么能读文件、跑命令、改代码，而且还能自己决定下一步”，很多解释就会重新掉回抽象口号里。

这一篇不谈大而空的框架口号，只回答最底层的问题：Agent 和普通对话式模型到底差在哪，工具调用到底是怎么接起来的，一个最小可运行闭环到底需要哪些部件。

本文通过逐行解读一个百行出头的极简 Agent 实现 `agent-essence.py`，把这些问题全部落回代码。

## 本文聚焦

- 对应脚本：[agent-essence.py](./agent-essence.py)
- 当前仓库中的脚本行数：`103`
- 这篇会回答三件事：
- Agent 和普通对话式 AI 的结构差异到底在哪里
- LLM 为什么能“调用工具”，而工具真正是谁在执行
- 一个最小可运行的 Agent 闭环究竟由哪些部件组成
...
这三个要素缺一不可。没有 LLM，就没有"思考"能力；没有工具，就无法作用于真实世界；没有循环，就做不了多步任务。接下来我们看 nanoagent 是怎么用一份百行出头的脚本把这三要素串起来的。

---

## 二、nanoagent 全局架构

nanoagent 的 `agent-essence.py` 在当前仓库中一共 `103` 行，但五脏俱全。整体结构可以拆成四个部分：

1. LLM 客户端初始化：准备模型调用入口。
2. 工具定义（Tool Schema）：告诉 LLM 它能调用哪些能力。
3. 工具实现（Tool Functions）：把工具名映射到真实 Python 函数。
4. Agent 循环（Core Loop）：持续让 LLM 决策、执行工具、读取结果，直到任务结束。
...
1. 把 `messages` 和 `tools` 一起发给 LLM。
2. LLM 判断下一步是直接回答，还是发起 `tool_call`。
3. 如果发起 `tool_call`，代码执行对应函数。
4. 工具结果以 `tool` 消息的形式追加回 `messages`。
5. 再次把完整历史发给 LLM，直到它返回纯文本并结束循环。
...
通过 nanoagent 这份百行出头的脚本，我们可以提炼出 Agent 的三个本质要素：
...
这些问题，我们在 [第二篇：记忆与规划](../02-memory/agent-memory.md) 中继续展开。到那时，Agent 不再只是“一次任务跑到底”的执行器，而会开始具备跨任务延续和多步拆解的能力。

---

*本文基于本仓库中的 `agent-essence.py` 分析。*
