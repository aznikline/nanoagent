...
很多人用过 ChatGPT、Claude 这样的对话式 AI，也听说过 AI Agent。但一旦问题变成“它为什么能读文件、跑命令、改代码，而且还能自己决定下一步”，解释往往又会重新滑回抽象口号。

这一篇不谈框架宣传，只回答最底层的问题：Agent 和普通对话式模型到底差在哪，工具调用是怎么接起来的，一个最小可运行闭环究竟需要哪些部件。

本文会逐行解读一个百行出头的极简实现 `agent-essence.py`，把这些问题全部落回代码。

## 本文聚焦

- 对应脚本：[agent-essence.py](./agent-essence.py)
- 当前仓库中的脚本行数：`103`
- 核心问题 1：Agent 和普通对话式 AI 的结构差异到底在哪里
- 核心问题 2：LLM 为什么能“调用工具”，而工具真正是谁在执行
- 核心问题 3：一个最小可运行的 Agent 闭环究竟由哪些部件组成
...
这些问题，我们在 [第二篇：记忆、规划与多步执行](../02-memory/agent-memory.md) 中继续展开。到那时，Agent 不再只是“一次任务跑到底”的执行器，而会开始具备跨任务延续和多步拆解的能力。
