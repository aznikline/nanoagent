...
- **description 写成触发条件**，而且写得比较"积极"——它的原话是 "pushy"。比如不只写"当用户要求创建仪表盘时触发"，还会加上"即使用户没有明确说'仪表盘'，只要提到了数据可视化、内部指标，都应该触发"。这和第二篇讲的"覆盖用户多种说法"一脉相承，只是更激进。
- **内容区分"参考知识"和"任务流程"**——该放进 references/ 的不会塞在 SKILL.md 里。
- **可复用的脚本放进 scripts/**——确定性的工作交给代码，不靠 AI 每次重写。

它还会特别提醒你：**写 Skill 是给另一个 Claude 实例看的，重点放在对 Claude 有用但不是显而易见的信息上。** 不需要教 Claude "什么是 API"，但需要告诉它"调用这个 API 时必须传 X-Auth-Token 头，否则返回 403"。
...
> "帮我优化 ~/.claude/skills/article-summarizer 这个 Skill，我觉得它的总结质量不太稳定。"
...
宁可多触发一次再判断不需要，也不要因为 description 太保守导致该触发的时候没触发。
...
skill-creator 在创建 Skill 时有一条很关键的提醒：你写的 Skill 是给另一个 Claude 实例使用的。重点应该放在"对 Claude 有用但不是显而易见的信息"上。

什么是"显而易见的"？Claude 已经知道怎么写 Python、怎么调用 REST API、怎么格式化 Markdown——这些不需要你在 Skill 里重复。
...
反过来，如果你理解了 Skill 的本质（第一篇）、知道每个部分怎么写才有效（第二篇）、经历过手工迭代的过程（第三篇）、明白 Skill 之间怎么配合（第四篇），再来用 skill-creator，你会发现——**它加速的是你本来就知道该怎么做的事情**。

意图捕获阶段，你知道该提供什么样的使用场景。起草阶段，你能判断它生成的 description 是否足够"推"。测试阶段，你能设计出有区分度的断言。迭代阶段，你能从 benchmark 数据里读出真正的问题。
